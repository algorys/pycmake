...
class CMakeLists(object):
    ...
    def init_file(self, path):
        ...
        try:
            ...
            sys.exit(
                'You do not have sufficient rights to write in '
                + path + '.'
            )
    ...
    def write_info(self):  # pragma: no cover
        ...
        if not self.cmakelists:
            sys.exit('CMakeLists must be init before !')
        else:
            self.cmakelists.write(
    ...
    def write_preprocessor_definitions(self, definitions):  # pragma: no cover
        ...
        if definitions:
            ...
            for index, definition in enumerate(definitions):  # pylint: disable=unused-variable
    ...
    def write_directory_files(self, sources_dirs):  # pragma: no cover
        ...
        for source_dir in sources_dirs:
            ...
            for index, source in enumerate(sources_dir):  # pylint: disable=unused-variable
    ...
    def write_dependencies(self, dependencies):  # pragma: no cover
        ...
        if dependencies.link_directories:
            ...
            for index, link in enumerate(link_directories):  # pylint: disable=unused-variable
                self.cmakelists.write(
        ...
        for target in targets:
            ...
            if project.sources_files:
                for source_file in project.sources_files:
                    ...
                    for index, src_file in enumerate(files):  # pylint: disable=unused-variable
                        if project.sources_files.get(source_file)['from_proj']:
                            self.cmakelists.write(
                                '    ${PROJECT_DIR}/' + src_file + '\n')
                        else:
                            self.cmakelists.write('    ' + src_file + '\n')
            self.cmakelists.write(')\n')
